...
def buy_similar(
    ...
    try:
        ...
        if len(small_world_args) > 0:
            args = []
            for arg, value in small_world_args.items():
                args.append(f"{arg}={value}")
            args = "&" + "&".join(args)
        else:
            args = ""
        query_url = f"{sw_server_path}submit?smi={smiles}&db={db}{args}"
    except:
        raise f"Failed to construct sw.docking.org query url for smiles '{smiles}'"
    ...
    if sw_status is None or sw_status != "END":
        raise Exception(f"Small World query did not complete, status: {sw_status}")

    try:
        hlid = int(hlid)
    except:
        raise Exception(f"Expected small world query id to be an inteter, instead it was {hlid}")
